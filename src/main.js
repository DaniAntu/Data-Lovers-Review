...
function printCard(element) {
  return `
      <div class="target col s3">
        <div class="card small ">
          <div class="card-image waves-effect waves-block waves-light center-align">
            <img class="activator" src=" ${element.img}">
          </div>
          <div class="card-content">
            <span class="card-title activator"><strong>${
              element.name
            }</strong><i class="material-icons right">more_vert</i></span>
            <p>#${element.num}</p>
          </div>
...
function createCardPoke(dataSent) {
  ...
  dataSent.forEach((element, index) => {
    if (number === 0 || index === number) {
      ...
      containerCards = document.getElementById('card' + index);
    } else {
...
spanValueFilter.forEach(function(element) {
  element.addEventListener('click', event => {
    ...
    if (valueSpan === 'All') {
      ...
      document.getElementById('calc').innerHTML =
        'Tienes un ' + calculation + '% de problabilidad de encontrar un pokemón, tipo ' + valueSpain;
                  ...
                  <div class="card small">
                        ...
                        <div class="card-reveal">
                              <span class="card-title"><strong>${
                                find[0].name
                              }</strong><i class="material-icons right">close</i></span>
